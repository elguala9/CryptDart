...
typedef InputHandler<T extends IExpiration> = ({
  T initialCrypt, 
  int maxCrypts,
  int? maxExpiredCrypts, // 0 = nessun limite, -1 = elimina tutti subito (quindi expiredCrypts sempre vuoto)
  int? maxDaysExpiredCrypts, // 0 = nessun limite, -1 = elimina tutti subito
});
...
abstract class Handler<T extends IExpiration> implements IHandler<T> {
  ...
  @protected
  void cleanCrypts() {
    // Sposta i crypts scaduti in protectedExpiredCrypts
    ...
    // Rimuovi expired troppo vecchi
    removeOldExpiredCrypts();
    // Limita il numero di expired
    limitExpiredCrypts();
  }
